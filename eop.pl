...
#设置远程路径
my %path=('remote'=>'/'
,'localdir'=>'/pardata/EDASCRIPT/publisher0/EOP/source/'
,'unpack'=>'/pardata/EDASCRIPT/publisher0/EOP/unpack/'
,'record'=>'/pardata/EDASCRIPT/publisher0/EOP/rec/'
,'log'=>'/pardata/EDASCRIPT/publisher0/EOP/log/');
...
##################################################################
{
	...
	my $ftp=defined;
	my %connStr=('host'=>'10.128.21.67','user'=>'GSBUS3602820190404','passwd'=>'K5Xj4SoXo11','port'=>'2121');
...
=cut
	 ...
	 my @downlist=();#下载文件列表
	 #获取当前日期
   my $current_date=strftime("%Y%m%d",localtime(time()-(3600*24)*2));
	 $ftp->cwd($remote) or die ("Can not into remote dir".$!."\n");#进入远程路径
	 my @list=$ftp->ls($remote);#获取远程路径下所有目录
		 foreach my $f (@list){
		 	  	if (substr($f,28,8)==$current_date){#如果目录中有当前日期目录
		 	  		msg("scan dir ".substr($f,28,8));
		 	  		push(@downlist,$f);
		 	  	}	
